...
public class MaxCommand extends MultiThreadCommand{
    public MaxCommand(ViewCalculation view) {
        super(view);
    }
    public int index = 1;
    ...
    public void execute() throws Exception {
        result = new TreeMap<>();
        System.out.println("Max Count executed...");
        ...
        for (var numberitem : viewCalculation.getItems()){
            ...
            for (var number_item_result : numberitem.getResult().entrySet()){
                if (number_item_result.getValue()>finalMax){
                    finalMax=number_item_result.getValue();
                    finalC = number_item_result.getKey();
                }
            }
            Character finalC1 = finalC;
            int finalMax1 = finalMax;
